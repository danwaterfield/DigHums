...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .home-link {{ color: #888; text-decoration: none; font-size: 15px; line-height: 1; padding: 0 4px; flex-shrink: 0; }}
...
// Open on the best-attested text rather than the earliest. The list stays in
// publication order, but a visitor's first view should be a substantial path,
// not the two- or three-point line a thinly-attested novel produces.
sel.value = TEXTS.reduce(
  (best, t, i) => t.events.length > TEXTS[best].events.length ? i : best, 0);
...
function onTextChange() {{
  ...
  if (coords.length > 0) {{
    // fitBounds against an unlaid-out container collapses to world zoom,
    // so make sure Leaflet has the real dimensions first.
    map.invalidateSize();
...
// ── initial render ─────────────────────────────────────────────────────────
// Wait for layout before the first fit, otherwise the map opens at world zoom.
