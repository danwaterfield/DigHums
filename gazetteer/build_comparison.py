...
@media (max-width: 760px) {
  body { height: 100dvh; }
  #topbar { max-height: 24vh; overflow-y: auto; gap: 6px; padding: 6px 8px; }
  #venue-select { min-width: 0; max-width: 100%; width: 100%; }
  #columns { flex-direction: column; }
  .col { min-height: 50%; border-right: none; border-bottom: 1px solid #ddd; }
...
const MODALITY_ORDER = ['auditory', 'olfactory', 'visual', 'thermal', 'crowd'];
...
function renderModBar(containerId, evs) {
  ...
  el.innerHTML = MODALITY_ORDER
    .filter(function(modality) { return counts[modality]; })
    .map(function(modality) {
      const n = counts[modality];
      const pct = (n / total * 100).toFixed(1);
      const col = MOD_COLORS[modality] || '#aaa';
      return '<div class="mod-seg mod-' + modality + '" style="width:' + pct + '%;background:' + col + '"'
           + ' title="' + modality + ': ' + n + ' (' + pct + '%)"></div>';
    }).join('');
}
