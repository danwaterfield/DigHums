...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: var(--bg-page); color: var(--text-primary); font-family: var(--font-sans); font-size: var(--text-base); line-height: 1.5; }
  header { padding: 14px 24px; background: var(--bg-surface); border-bottom: 1px solid var(--border-default); display: flex; align-items: baseline; gap: 12px; }
  header h1 { font-family: var(--font-serif); font-size: var(--text-lg); font-weight: 600; letter-spacing: -0.01em; }
  header p { color: var(--text-secondary); font-size: var(--text-sm); }

  /* Tabs */
  #tab-bar { display: flex; gap: 2px; padding: 8px 24px 0; border-bottom: 1px solid var(--border-default); background: var(--bg-surface); }
  .tab-btn { padding: 6px 16px; border: 1px solid transparent; border-bottom: none; border-radius: var(--radius-sm) var(--radius-sm) 0 0; background: transparent; color: var(--text-muted); cursor: pointer; font-size: var(--text-sm); font-family: var(--font-sans); transition: background 0.15s, color 0.15s; letter-spacing: 0.02em; }
  .tab-btn:hover { background: var(--bg-control); color: var(--text-primary); }
  .tab-btn.active { background: var(--bg-page); color: var(--text-primary); border-color: var(--border-default); font-weight: 600; }
  ...
  .controls label { color: var(--text-secondary); font-size: var(--text-sm); display: flex; align-items: center; gap: 6px; }
  .controls select { background: var(--bg-surface); color: var(--text-primary); border: 1px solid var(--border-default); border-radius: var(--radius-sm); padding: 4px 8px; font-size: var(--text-sm); font-family: var(--font-sans); }
  .controls input[type=range] { accent-color: var(--accent-primary); width: 100px; }
  .controls input[type=checkbox] { accent-color: var(--accent-primary); }

  /* Pills */
  .pills { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 16px; }
  .pill { padding: 3px 10px; border-radius: var(--radius-pill); border: 1px solid var(--border-default); background: var(--bg-surface); color: var(--text-secondary); cursor: pointer; font-size: var(--text-xs); font-family: var(--font-sans); transition: opacity 0.15s, background 0.15s; }
  .pill.active { opacity: 1; background: var(--bg-control); border-color: var(--accent-primary); color: var(--text-primary); }
  .pill.inactive { opacity: 0.35; }

  /* Canvas wrapper */
  .chart-wrap { position: relative; width: 100%; background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 8px; }
  canvas { display: block; }

  /* Novel grid */
  #grid-container { display: grid; grid-template-columns: repeat(auto-fill, minmax(190px, 1fr)); gap: 12px; margin-top: 12px; }
  .novel-card { background: var(--bg-surface); border: 1px solid var(--border-default); border-radius: var(--radius-md); padding: 12px; cursor: pointer; transition: box-shadow 0.15s; }
  .novel-card:hover { box-shadow: var(--shadow-md); }
  .card-title { font-family: var(--font-serif); font-size: var(--text-sm); font-weight: 600; margin-bottom: 2px; color: var(--text-primary); }
  .card-meta { font-size: var(--text-xs); color: var(--text-muted); margin-bottom: 8px; }
  .card-bar { display: flex; height: 5px; border-radius: 3px; overflow: hidden; width: 100%; }
  ...
  .panel-title { font-family: var(--font-serif); font-size: var(--text-md); font-weight: 600; margin-bottom: 10px; color: var(--text-primary); }
  .callout { background: var(--accent-prose-bg); border-left: 3px solid var(--accent-prose-border); border-radius: 0 var(--radius-md) var(--radius-md) 0; padding: 14px 18px; margin-top: 24px; font-size: var(--text-sm); line-height: 1.65; color: var(--accent-prose-text); }
  .callout strong { color: var(--text-primary); }

  /* Tooltip */
  #tooltip { position: fixed; background: var(--bg-surface); border: 1px solid var(--border-default); border-radius: var(--radius-md); padding: 8px 12px; font-size: var(--text-xs); pointer-events: none; z-index: 1000; display: none; max-width: 220px; line-height: 1.5; box-shadow: var(--shadow-md); color: var(--text-primary); }
</style>
</head>
<body>
...
(function () {
  ...
  function genreColour(g) { return GENRE_COLOURS[g] || "#9c9890"; }
  ...
  function drawCentury() {
    ...
    if (reg) {
      ...
      ctx.strokeStyle = "rgba(30,60,110,0.4)";
      ctx.lineWidth = 1.5;
      ctx.setLineDash([4, 4]);
      ctx.beginPath();
  ...
  function drawArcs() {
    ...
    if (dominantOnly) {
      ...
      for (let ci = CATS.length - 1; ci >= 0; ci--) {
        ...
        ctx.closePath();
        ctx.fillStyle = CAT_COLOURS[cat] + "d0";
        ctx.fill();
      }
    }
    ...
    if (showBounds && novel.volume_boundaries && novel.volume_boundaries.length) {
      novel.volume_boundaries.forEach(function(b) {
        const bx = PAD.l + b * CW;
        ctx.strokeStyle = "rgba(30,60,110,0.5)";
        ctx.lineWidth = 1.5;
        ctx.setLineDash([5, 5]);
        ctx.beginPath();
        ctx.moveTo(bx, PAD.t);
  ...
  function drawEcoScatter() {
    ...
    if (reg) {
      const x0 = 0, x1 = maxX;
      ctx.strokeStyle = "rgba(30,60,110,0.35)";
      ctx.lineWidth = 1.5;
      ctx.setLineDash([4, 4]);
      ctx.beginPath();
      ctx.moveTo(px(x0), py(reg.m * x0 + reg.b));
  ...
  function drawEcoEnv() {
    ...
    ENV.forEach(function(e, i) {
      ...
      ctx.fillStyle = "#b8742ecc";
      ctx.fillRect(bx(i), by(e.smell), barW, smellH);

      ctx.fillStyle = "#7a8070cc";
